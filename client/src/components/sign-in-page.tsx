...
export function SignInPage() {
  ...
  const handleCredentials = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      setError(tab === "register" ? "Account created! Please sign in." : "Invalid email or password");
    ...
    <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center bg-amber-50/40 px-4">
      <div className="w-full max-w-sm">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 py-3 px-4 border border-amber-200 rounded-xl bg-white hover:bg-amber-50 transition-colors text-amber-900 font-display text-sm mb-4"
        >
        ...
        {/* Tabs */}
        <div className="flex mb-4 border border-amber-200 rounded-xl overflow-hidden">
          <button
            onClick={() => { setTab("signin"); setError(""); }}
            className={`flex-1 py-2 text-sm font-display transition-colors ${tab === "signin" ? "bg-amber-100 text-amber-900" : "text-amber-600/60 hover:text-amber-700"}`}
          >
            Sign in
          </button>
          <button
            onClick={() => { setTab("register"); setError(""); }}
            className={`flex-1 py-2 text-sm font-display transition-colors ${tab === "register" ? "bg-amber-100 text-amber-900" : "text-amber-600/60 hover:text-amber-700"}`}
          >
            Create account
          </button>
        </div>
        ...
        <form onSubmit={handleCredentials} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white font-display rounded-xl transition-colors"
          >
